...
This specification defines deterministic rollback proofs for identity graph integrity in the NewZone standard.  
Its purpose is to ensure that rolled‑back identity graphs remain provably intact, immutable, and formally verifiable.
...
- deterministic rollback proofs for identity graphs (NZ‑0035)  
- compatibility with identity graph integrity proofs (NZ‑0040)  
- integration with replay proofs (NZ‑0048)  
- constraints ensuring predictability and formal verifiability  

This specification does not define metadata rollback proofs (NZ‑0053).

---

3. Terminology

Identity Graph Integrity Rollback Proof — deterministic evidence that a rolled‑back identity graph maintains integrity.  
...
Each identity graph rollback proof is defined as:

```
identitygraphrollbackproof(graphid) = identitygraphintegrity(graphid, fixedparameters)
```
...
Proof must bind graph_id to identity graph rollback.
