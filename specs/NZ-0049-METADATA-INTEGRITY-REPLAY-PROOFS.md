...
This specification defines deterministic replay proofs for metadata integrity in the NewZone standard.  
Its purpose is to ensure that replayed metadata blocks remain provably intact, immutable, and formally verifiable.
...
- deterministic replay proofs for metadata (NZ‑0045)  
- compatibility with metadata integrity proofs (NZ‑0037)  
- integration with metadata compression proofs (NZ‑0041)  
- constraints ensuring predictability and formal verifiability  

This specification does not define consensus replay proofs (NZ‑0050).

---

3. Terminology

Metadata Integrity Replay Proof — deterministic evidence that replayed metadata maintains integrity.  
...
Each metadata integrity replay proof is defined as:

```
metadataintegrityreplayproof(metadataid) = metadataintegrity(metadataid, fixed_parameters)
```
...
Proof must bind metadata_id to metadata replay.
