...
This specification defines deterministic rollback proofs for consensus integrity in the NewZone standard.  
Its purpose is to ensure that rolled‑back consensus decisions remain provably intact, immutable, and formally verifiable.
...
- deterministic rollback proofs for consensus (NZ‑0038)  
- compatibility with consensus integrity proofs (NZ‑0046)  
- integration with consensus replay proofs (NZ‑0050)  
- constraints ensuring predictability and formal verifiability  

This specification does not define transport rollback proofs (NZ‑0055).

---

3. Terminology

Consensus Integrity Rollback Proof — deterministic evidence that a rolled‑back consensus maintains integrity.  
...
Each consensus rollback proof is defined as:

```
consensusrollbackproof(consensusid) = consensusintegrity(consensusid, fixedparameters)
```
...
Proof must bind consensus_id to consensus rollback.
