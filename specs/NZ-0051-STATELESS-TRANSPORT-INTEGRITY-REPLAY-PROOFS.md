...
This specification defines deterministic replay proofs for stateless transport integrity in the NewZone standard.  
Its purpose is to ensure that replayed transport integrity checks remain provably intact, immutable, and formally verifiable.
...
- deterministic replay proofs for transport integrity (NZ‑0043)  
- compatibility with stateless transport replay proofs (NZ‑0047)  
...
This specification does not define identity graph rollback proofs (NZ‑0052).

---

3. Terminology

Transport Integrity Replay Proof — deterministic evidence that a replayed transport integrity check is valid.  
...
Each transport integrity replay proof is defined as:

```
transportintegrityreplayproof(flowid) = transportintegrity(flowid, fixed_parameters)
```
...
Proof must bind flow_id to transport integrity replay.
